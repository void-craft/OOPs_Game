...
export class CollisionManager {
  ...
  handleCoinCollision(coin, index) {
    ...
    animationElement.style.left = `${
      coinRect.left - containerRect.left + coinRect.width / 2 - 12.5
    }px`; // Centering the 25px icon
    animationElement.style.top = `${
      coinRect.top - containerRect.top - 30 // Adjust this value to move the icon above the coin
    }px`;
  ...
  handleObstacleCollision(index) {
    ...
    if (this.game.character && this.game.character.element) {
      ...
      // Calculate position relative to the container
      animationElement.style.left = `${
        charRect.left - containerRect.left + charRect.width / 2 - 12.5
      }px`; // Centering the 25px icon
      animationElement.style.top = `${
        charRect.top - containerRect.top - 30 // Adjust this value to move the icon above the character
      }px`;
